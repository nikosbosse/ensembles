### Questions regarding Nikos helper functions. 
#### Interesting take is how to manipulate data from here. 
...
### Trying to solve how mutate calculates n for each location. 
data <- tibble(hub_data)
data <- data |> group_by(model, forecast_date) |> mutate(n = length(unique(location)))




## Nikos visualizing functions
